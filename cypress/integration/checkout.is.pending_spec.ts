import { retriesDefault, acceptAndSubmitConditionsForm, fillAndSubmitPersonalDataForm, fillOneTimePasswordForm, uploadDocumentation, data } from "../helpers/checkout";
...
describe('Aplazame - Checkout PENDING', () => {
    ...
    context('Modal Checkout', () =>{
        ...
        it("should verify identity uploading documentation to approve funding", retriesDefault, () => {
            uploadDocumentation()
            cy.enter('#aplazame-checkout-iframe', { timeout: 10000 }).then(getBody => {
                getBody().find('modal-upload-documentation').as('modalUploadDocumentation')
                cy.get('@modalUploadDocumentation').find('div[message="challenges.upload_files.attached"]').should('be.visible').and('have.length',2)
            })
        })
